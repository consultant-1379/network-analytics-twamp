...
function setRstate {
	...
	mv twamp/build/feature-release.xml twamp/build/feature-release.${rstate}.xml
		echo "Building rstate:$rstate"
}
function getRelease {
	        release=`cat $PWD/build.cfg | grep $module | grep $branch | awk -F " " '{print $5}'`
}
...
function Arm104nexusDeploy {
	...
	ArtifactId=$module
	zipName=Ericsson-TWAMP-Analysis
	
	
	echo "****"	
	echo "Deploying the zip /$zipName-$release.zip as ${ArtifactId}${rstate}.zip to Nexus...."
        mv target/$zipName-$release.zip target/${ArtifactId}.zip
	echo "****"	
